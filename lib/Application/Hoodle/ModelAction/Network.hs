...
module Application.Hoodle.ModelAction.Network where
...
import Application.Hoodle.NetworkClipboard.Client.Config
import Application.Hoodle.NetworkClipboard.Type
import Data.UUID
import Data.UUID.V5
...
startCreate mc strs = do 
  ...
  uuid <- nextUUID mc
  let info = HoodleClipInfo { hoodleclip_uuid = uuid 
